...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        @php
            $stats = [
                ...
                ['label' => 'Dépenses', 'value' => $colocation->depenses->count(), 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'color' => 'bg-amber-50 text-amber-600'],
                ['label' => 'Total dépenses', 'value' => number_format($colocation->depenses->sum('montant'), 2).' €', 'icon' => 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z', 'color' => 'bg-emerald-50 text-emerald-600'],
            ];
        @endphp
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <a href="{{ route('depenses.create', ['colocation' => $colocation->id]) }}"
        ...
        <div class="divide-y divide-slate-100">
            @forelse($colocation->depenses->take(8) as $depense)
                <div class="flex items-center gap-4 px-6 py-3.5">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-slate-700 truncate">{{ $depense->title }}</p>
                        <p class="text-xs text-slate-400">{{ $depense->user->name }} · {{ $depense->created_at->format('d M Y') }}</p>
                    </div>
                    <div class="text-right flex-shrink-0">
                        <p class="text-sm font-semibold text-slate-800">{{ number_format($depense->montant, 2) }} €</p>
                        @if($depense->categories)
                            <span class="text-xs text-slate-400">{{ $depense->category->name }}</span>
                        @endif
                    </div>
                </div>
            @empty
        ...
        @if($colocation->depenses->count() > 8)
            <div class="px-6 py-3 bg-slate-50 border-t border-slate-100">
                <a href="{{ route('depenses.index', ['colocation' => $colocation->id]) }}"
