...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        @php
            $stats = [
                ['label' => 'Membres', 'value' => $colocation->members->count(), 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z', 'color' => 'bg-blue-50 text-blue-600'],
                ['label' => 'Dépenses', 'value' => $colocation->expenses->count(), 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'color' => 'bg-amber-50 text-amber-600'],
                ['label' => 'Total dépenses', 'value' => number_format($colocation->expenses->sum('amount'), 2).' €', 'icon' => 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z', 'color' => 'bg-emerald-50 text-emerald-600'],
            ];
        @endphp
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span class="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full">{{ $colocation->members->count() }}</span>
                </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('expenses.create', ['colocation' => $colocation->id]) }}"
                ...
                <div class="divide-y divide-slate-100">
                    @forelse($colocation->expenses->take(8) as $expense)
                        <div class="flex items-center gap-4 px-6 py-3.5">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-slate-400">{{ $expense->user->name }} · {{ $expense->created_at->format('d M Y') }}</p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <p class="text-sm font-semibold text-slate-800">{{ number_format($expense->amount, 2) }} €</p>
                                @if($expense->category)
                                    <span class="text-xs text-slate-400">{{ $expense->category->name }}</span>
                                @endif
                            </div>
                        </div>
                    @empty
                ...
                @if($colocation->expenses->count() > 8)
                    <div class="px-6 py-3 bg-slate-50 border-t border-slate-100">
                        <a href="{{ route('expenses.index', ['colocation' => $colocation->id]) }}"
