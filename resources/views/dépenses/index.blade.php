...
@section('content')
    ...
    <form method="GET" class="flex flex-wrap items-center gap-3 mb-6 bg-white rounded-2xl border border-slate-200 px-5 py-4">
        ...
        @if(request()->hasAny(['search','colocation_id','category_id']))
            <a href="{{ route('dépenses.index') }}" class="text-sm text-slate-500 hover:text-slate-700 transition-colors">Réinitialiser</a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 class="font-semibold text-slate-800 text-sm">{{ $dépenses->total() ?? 0 }} dépense(s)</h3>
        ...
        @else
                        ...
                        @foreach($dépenses as $dépense)
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span class="text-sm font-semibold text-slate-800">{{ number_format($dépense->amount, 2) }} €</span>
                                </td>
                                <td class="px-6 py-4">
                                    <form method="POST" action="{{ route('dépenses.destroy', $dépense) }}" onsubmit="return confirm('Supprimer cette dépense ?')">
                                        @csrf @method('DELETE')
            ...
            <div class="px-6 py-4 border-t border-slate-100">
                {{ $dépenses->withQueryString()->links() }}
            </div>
        @endif
    </div>
